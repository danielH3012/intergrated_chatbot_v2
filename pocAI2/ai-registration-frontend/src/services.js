...
// Uses relative URLs so Vite proxy (or same-origin deploy) handles routing to the backend.
...
export async function registerUser({ username, email, password, role, company, id_perusahaan }) {
...
export async function createAssetManual(data) {
...
export function downloadAssets(params = {}) {
  const q = new URLSearchParams();
  if (params.search) q.append('search', params.search);
  window.open(`${API_BASE}/api/assets/download?${q.toString()}`, '_blank');
}

export function downloadAssetsCSV(assets = []) {
...
export async function startRegistrationSession({ file, rawText }) {
...
export async function updateDraftField(sessionId, draftId, fieldKey, value) {
...
export async function discardDraft(sessionId, draftId) {
  const res = await fetch(`${API_BASE}/api/ai-registration/drafts/${encodeURIComponent(draftId)}`, {
    method: 'DELETE',
    headers: getAuthHeaders(),
  });
...
export async function confirmAll(sessionId) {
