...
function AuthPage({ onAuthSuccess, showToast }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!trimmedUser) { setError('Username or email is required'); return }
    if (!password) { setError('Password is required'); return }
    if (mode === 'signup') {
      const trimmedEmail = email.trim()
      if (!trimmedEmail) { setError('Email address is required'); return }
      if (!trimmedEmail.includes('@') || !trimmedEmail.includes('.')) { setError('Please enter a valid email address'); return }
      ...
      <div className="auth-card">
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          <div className="form-row">
            <label>{mode === 'login' ? 'Username or Email' : 'Username'} <span className="req">*</span></label>
            ...
            <>
              ...
              <div className="form-row">
                ...
                <div className="input-icon-wrap">
                  ...
                  <select value={role} onChange={(e) => setRole(e.target.value)} disabled={loading} className="input-with-icon">
        ...
        {mode === 'login' && (
          <div className="auth-quick-fill-section">
            ...
            <div className="auth-quick-fill-chips">
              <button type="button" className="chip-quick-fill" onClick={() => quickFill('admin', 'admin123')}>
                <i className="ph ph-shield-check" /> admin / admin123
              </button>
              <button type="button" className="chip-quick-fill" onClick={() => quickFill('operator', 'operator123')}>
        ...
        <div className="auth-card-footer">
          {mode === 'login' ? (
            <p>
              ...
              <button type="button" className="link-btn" onClick={() => { setMode('signup'); setError('') }}>Sign Up</button>
            </p>
          ) : (
            <p>
              Already have an account?{' '}
              <button type="button" className="link-btn" onClick={() => { setMode('login'); setError('') }}>Sign In</button>
            </p>
          )}
        </div>
      </div>
...
function AssetListPage({ onRegister, onEdit, onChat, showToast, reloadKey }) {
  ...
  useEffect(() => { loadOptions() }, [loadOptions, reloadKey])
...
function RegistrationFlow({ onDone, onManual }) {
  return (
    <div className="modal-overlay">
      <div className="modal">
