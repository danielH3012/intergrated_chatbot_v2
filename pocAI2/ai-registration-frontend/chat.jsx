...
export default function Chat() {
  ...
  const toggleSpeak = async (id, text) => {
    ...
    const isId = /\b(yang|dan|di|ini|itu|ada|tidak|untuk|dengan|dari|pada|ke|aset|adalah|sudah|bisa|perusahaan|pengguna|selamat|pagi|siang|sore|malam|terima\s*kasih)\b/i.test(cleanText)
    ...
    try {
      ...
      if (resp.ok) {
        ...
        console.error('[Cartesia TTS] Backend error:', resp.status, errJson)
      }
    } catch (err) {
      console.error('[Cartesia TTS] Failed to fetch audio from backend:', err)
    } finally {
      setLoadingTtsId(null)
    }
  }
  ...
  const toggleRecording = async () => {
    if (isRecording) {
      ...
      try {
        ...
        let text = ''
        const elevenKey = import.meta.env.VITE_ELEVENLABS_API_KEY || import.meta.env.ELEVENLABS_API_KEY
        const scribeModel = import.meta.env.VITE_ELEVENLABS_SCRIBE_MODEL || 'scribe_v2'

        // Method 1: Direct client-side ElevenLabs Scribe v2 API call (zero backend hop)
        if (elevenKey) {
          try {
            const formData = new FormData()
            formData.append('file', wavBlob, 'recording.webm')
            formData.append('model_id', scribeModel)

            // Language setting: if 'id' or 'en', specify language_code.
            // If 'auto', omit language_code so Scribe v2 automatically detects Indonesian & English!
            ...
            if (res.ok) {
              ...
              console.warn('[ElevenLabs Scribe v2] Direct API error:', res.status, errData)
            }
          } catch (err) {
            console.warn('[ElevenLabs Scribe v2] Direct fetch failed, trying backend fallback:', err)
          }
        }

        // Method 2: Fallback to backend /transcribe if direct call failed or key missing
        if (!text) {
          const formData = new FormData()
        ...
        if (text) {
          // Clean non-speech annotations e.g. [silence], [blank]
          const cleanText = text.replace(/^\[(silence|blank|cough|sigh|music|applause)\]$/i, '').trim()
          if (cleanText) {
          ...
          alert('Failed to transcribe audio. Please ensure microphone permissions and ElevenLabs API key are active.')
        }
      } catch (err) {
      ...
      <div className="chat-app">
        ...
        <footer className="chat-composer-wrap">
          ...
          <div className="chat-composer">
            ...
            {isRecording ? (
              ...
              <div className="recording-status-pill" style={{ flex: 1, color: '#fde047', borderColor: '#eab308' }}>
                <span>⏳ Transcribing with ElevenLabs Scribe v2 ({sttLang === 'id' ? 'ID' : sttLang === 'en' ? 'EN' : 'Auto ID/EN'})...</span>
              </div>
            ) : (
              <input
                ref={inputRef}
          ...
          <select
            ...
            title="Speech Recognition Language (ElevenLabs Scribe v2)"
          >
            <option value="auto">🌐 Auto (ID/EN)</option>
